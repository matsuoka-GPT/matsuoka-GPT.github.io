...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--page-size", type=int, default=DEFAULT_PAGE_SIZE, help="Results per page. Keep at 25 for unauthenticated GitHub Actions runs.")
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            if isinstance(exc, urllib.error.HTTPError) and exc.code < 500 and exc.code != 429:
                ...
                raise RuntimeError(f"Zenodo API request failed: HTTP {exc.code} {url}: {error_body}") from exc
...
def iter_records(author: str, page_size: int, api_url: str) -> Iterable[dict[str, Any]]:
    params: dict[str, Any] = {
        # Zenodo's current search guide lists `creators.name` as a searchable
        # field and shows quoted phrases as valid query syntax. The GitHub
        # Actions HTTP 400 was caused by requesting `size=100` anonymously, not
        # by this query field.
