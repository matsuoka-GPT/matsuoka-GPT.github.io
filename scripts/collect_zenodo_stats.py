...
def metric_card(label: str, value: int, delta: int | None = None) -> str:
    ...
    return f'<article class="metric-card pale-blue-surface metallic-surface"><span>{safe(label)}</span><strong>{value:,}</strong>{delta_html}</article>'
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    header {{ display:flex; align-items:center; min-height:46px; padding: 6px 108px 16px 0; }}
    .brand-link {{ display:inline-block; max-width:100%; color:var(--accent); font-weight:900; letter-spacing:-.02em; line-height:1.35; text-decoration:none; border-radius:6px; }}
    .brand-link:hover {{ color:var(--accent2); text-decoration:underline; text-decoration-thickness:2px; text-underline-offset:4px; }}
    .brand-link:focus-visible {{ outline:2px solid var(--accent); outline-offset:4px; }}
    ...
    .metric-card {{ border-radius:var(--radius2); padding:12px; }}
    ...
    .record-dialog {{ width:min(520px, calc(100% - 136px)); max-height:calc(100vh - 32px); padding:0; overflow:visible; border:1px solid var(--border-color); border-radius:var(--radius); color:var(--text-color); background:var(--card-color); box-shadow:var(--shadow2); }}
    ...
    .dialog-inner {{ padding:18px; max-height:calc(100vh - 32px); overflow:auto; border-radius:inherit; }} .dialog-header {{ display:flex; align-items:flex-start; justify-content:space-between; gap:12px; }}
    ...
    .dialog-nav {{ position:absolute; top:50%; width:40px; height:48px; border:1px solid var(--border-color); border-radius:10px; transform:translateY(-50%); background:var(--control-surface-color); color:var(--link-color); font:inherit; font-size:22px; line-height:1; cursor:pointer; }}
    .dialog-nav:hover:not(:disabled) {{ color:var(--heading-color); border-color:var(--link-color); }}
    .dialog-nav:focus-visible {{ outline:2px solid var(--link-color); outline-offset:2px; }}
    .dialog-nav:disabled {{ opacity:.32; cursor:not-allowed; }}
    .dialog-prev {{ left:-56px; }} .dialog-next {{ right:-56px; }}
    ...
    @media (max-width: 420px) {{ header {{ padding-top:48px; padding-right:0; }} }}
    @media (max-width: 560px) {{ .record-dialog {{ width:calc(100% - 104px); max-height:calc(100vh - 24px); }} .dialog-inner {{ padding:16px; }} .dialog-nav {{ position:fixed; width:36px; height:44px; }} .dialog-prev {{ left:8px; }} .dialog-next {{ right:8px; }} }}
    @media (max-width: 360px) {{ .dialog-stats {{ grid-template-columns:1fr; }} }}
  </style>
</head>
<body>
  <main class=\"wrap\">
    ...
    <section class=\"hero pale-blue-surface metallic-surface\">
  ...
  <dialog class="record-dialog" id="record-dialog" aria-labelledby="record-dialog-title">
    <button class="dialog-nav dialog-prev" type="button" aria-label="Previous paper in download ranking" title="Previous paper">&#9664;</button>
    <button class="dialog-nav dialog-next" type="button" aria-label="Next paper in download ranking" title="Next paper">&#9654;</button>
    <div class="dialog-inner">
      <div class="dialog-header"><h2 class="dialog-title" id="record-dialog-title" aria-live="polite"></h2><button class="dialog-close" type="button" aria-label="Close paper statistics">&times;</button></div>
      <p class="dialog-meta"><span data-detail="ranking_position"></span> · <span data-detail="category"></span> · <time data-detail="publication_date"></time></p>
  ...
  <script>
    (() => {{
      ...
      const rankingButtons = [...document.querySelectorAll('.paper-title')];
      const rankingIds = rankingButtons.map((button) => button.dataset.recordId);
      let opener = null;
      ...
      const showRankingRecord = (index) => {{
        const record = details[rankingIds[index]];
        if (!record) return;
        currentRankingIndex = index;
        dialog.querySelector('#record-dialog-title').textContent = record.title;
        dialog.querySelector('[data-detail="ranking_position"]').textContent = `Download rank ${{index + 1}} of ${{rankingIds.length}}`;
        ...
        nextButton.disabled = index === rankingIds.length - 1;
      }};
      rankingButtons.forEach((button, index) => button.addEventListener('click', () => {{
        opener = button;
        showRankingRecord(index);
      ...
      previousButton.addEventListener('click', () => showRankingRecord(currentRankingIndex - 1));
      nextButton.addEventListener('click', () => showRankingRecord(currentRankingIndex + 1));
      ...
      document.addEventListener('keydown', (event) => {{
        ...
        else if (event.key === 'ArrowLeft' && currentRankingIndex > 0) {{ event.preventDefault(); showRankingRecord(currentRankingIndex - 1); }}
        else if (event.key === 'ArrowRight' && currentRankingIndex < rankingIds.length - 1) {{ event.preventDefault(); showRankingRecord(currentRankingIndex + 1); }}
      }});
    }})();
  </script>
</body>
