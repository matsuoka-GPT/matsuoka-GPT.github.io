...
SCRIPT = ROOT / "scripts" / "collect_zenodo_stats.py"
...
def test_default_page_size_is_safe_for_unauthenticated_actions():
    spec = importlib.util.spec_from_file_location("collect_zenodo_stats", SCRIPT)
    module = importlib.util.module_from_spec(spec)
    sys.modules["collect_zenodo_stats"] = module
    assert spec.loader is not None
    spec.loader.exec_module(module)
    assert module.DEFAULT_PAGE_SIZE == 25
    url = module.build_url("https://zenodo.org/api/records", {"q": 'creators.name:"Takafumi Matsuoka"', "size": module.DEFAULT_PAGE_SIZE})
    assert "creators.name%3A%22Takafumi+Matsuoka%22" in url
